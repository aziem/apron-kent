 ...
 */
oct_t* oct_set_mat(oct_internal_t* pr, oct_t* a, dbm* m, dbm* closed,
		   bool destructive)
{
  oct_t* r;
  if (destructive) {
		  ...
		  ap_interval_t ** t)
{
  ...
  for (i=0;i<r->dim;i++)
    if (bounds_of_interval(pr,
			   *getdbm(r->closed,matpos(2*i,2*i+1)),
			   *getdbm(r->closed,matpos(2*i+1,2*i)),
			   t[i],true)) {
			      ...
			      ap_generator0_array_t* ar)
{
  ...
  for (i=0;i<ar->size;i++) {
    ...
    for (j=0;j<2*dim;j++)
      for (k=0;k<=(j|1);k++) {
	bound_sub(*getdbm(r->m,matpos2(j,k)),pr->tmp[j],pr->tmp[k]);
      }
    break;
  }
...
{
  //fprintf(stderr, "OCT CACHE CLOSURE: ");
  //if (a->closed || !a->m) { fprintf(stderr, "Not closing\n"); return; }
  if (a->closed || !a->m) return;
  /* fprintf(stderr, "Closing\n"); */
  a->closed = hmat_copy(pr,a->m,a->dim);
  if (hmat_close(pr, a->closed,a->dim)) {
    /* empty! */
    hmat_free(pr,a->m,a->dim);
    hmat_free(pr,a->closed,a->dim);
    a->m = a->closed = NULL;
...
int oct_hash(ap_manager_t* man, oct_t* a)
{
  ...
  if (a->closed || a->m) {
    ...
    for (i=0;i<2*a->dim;i++)
      for (j=0;j<=(i|1);j++)
	r = r*37 + bound_hash(*getdbm(m,matpos2(i,j)));
    return r;
  }
  return 0;
}
