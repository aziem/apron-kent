				    ...
				    char** name_of_dim)
{
  ...
  for (i=0;i<2*dim;i++)
    for (j=0;j<=(i|1);j++) {
      ap_lincons0_t l;
      if (i==j || bound_infty(*getdbm(d,matpos2(i,j)))) continue;
      l = lincons_of_bound(pr,i,j,*getdbm(d,matpos2(i,j)));
		    ...
		    oct_t* a1, oct_t* a2, char** name_of_dim)
{
  ...
  if (!a1->closed && !a1->m) {
    ...
    for (i=0;i<2*a1->dim;i++)
      for (j=0;j<=(i|1);j++) {
	ap_lincons0_t l;
	if (i==j || !bound_cmp(*getdbm(m1, matpos2(i,j)),*getdbm(m2, matpos2(i,j)))) continue;
	
	l = lincons_of_bound(pr,i,j,*getdbm(m1, matpos2(i,j)));
	ap_lincons0_fprint(stream,&l,name_of_dim);
	ap_lincons0_clear(&l);

	fprintf(stream," / ");

	l = lincons_of_bound(pr,i,j,*getdbm(m2, matpos2(i,j)));
	ap_lincons0_fprint(stream,&l,name_of_dim);
	ap_lincons0_clear(&l);
...
ap_membuf_t oct_serialize_raw(ap_manager_t* man, oct_t* a)
{
  ...
  if (a->m) 
    ...
    n += dbm_serialized_size_array(a->m,matsize(a->dim));
  else if (a->closed) 
    n += dbm_serialized_size_array(a->closed,matsize(a->dim));
  ...
  if (a->m) {
    ((char*)buf.ptr)[1] = 1;
    buf.size = 10 + dbm_serialize_array
...
oct_t* oct_deserialize_raw(ap_manager_t* man, void* ptr, size_t* size)
{
  ...
  case 1:
    ...
    *size = 10 + dbm_deserialize_array(r->m,(char*)ptr+10,matsize(dim));
    break;
  case 2:
    r->closed = hmat_alloc(pr,dim);
    *size = 10 + dbm_deserialize_array(r->closed,(char*)ptr+10,matsize(dim));
    break;
  default: assert(0);
  }
  return r;
