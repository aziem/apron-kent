...
bool oct_is_top(ap_manager_t* man, oct_t* a)
{
  ...
  size_t i,j;
  dbm* m = a->m ? a->m : a->closed;
  if (!m) return false;
  for (i=0;i<2*a->dim;i++)
    for (j=0;j<=(i|1);j++)
      if (!bound_infty(*getdbm(m, matpos(i,j))) && i!=j)
	return false;
  return true;
}
...
bool oct_is_leq(ap_manager_t* man, oct_t* a1, oct_t* a2)
{
  ...
  if (!a1->closed && !a1->m) {
    ...
    size_t i,j;
    dbm *x = a1->closed ? a1->closed : a1->m;
    dbm *y = a2->closed ? a2->closed : a2->m;
    for (i=0;i<2*a1->dim;i++)
      for (j=0;j<=(i|1);j++)
	if (bound_cmp(*getdbm(x, matpos2(i,j)),*getdbm(y,matpos2(i,j)))>0) {
	  if (a1->closed) {
	    /* not included on Q */
	    if (num_incomplete || a1->intdim) { flag_incomplete; }
	    return false;
...
bool oct_is_eq(ap_manager_t* man, oct_t* a1, oct_t* a2)
{
  ...
  if (!a1->closed && !a1->m) {
    ...
    for (i=0;i<2*a1->dim;i++)
      for (j=0;j<=(i|1);j++)
	if (bound_cmp(*getdbm(x, matpos2(i,j)),*getdbm(y,matpos2(i,j)))) {
	  if (a1->closed) {
	    /* not equal on Q */
		      ...
		      ap_dim_t dim, ap_interval_t* i)
{
  ...
  if (!a->closed && !a->m) {
    ...
    interval_of_bounds(pr,b,
		       *getdbm(m,matpos(2*dim,2*dim+1)),*getdbm(m,matpos(2*dim+1,2*dim)),true);
    /* compare with i */
				    ...
				    ap_dim_t dim)
{
  ...
  if (!a->closed && !a->m)
    ...
    dbm* b = a->closed ? a->closed : a->m;
    size_t i, d2=2*dim;
    for (i=0;i<2*a->dim;i++) {
      if (!bound_infty(*getdbm(b,matpos2(i,d2))) && i!=d2) return false;
      if (!bound_infty(*getdbm(b,matpos2(i,d2+1))) && i!=d2+1) return false;
    }
    return true;
  }
}
		     ...
		     ap_lincons0_t* lincons)
{
  ...
  if (!a->closed && !a->m) {
   ...
   case UNARY:
      ...
      bound_badd(pr->tmp[0],*getdbm(b,matpos(ui,ui^1)));
      bound_badd(pr->tmp[1],*getdbm(b,matpos(ui^1,ui)));
    ...
    case BINARY:
      ...
      bound_badd(pr->tmp[0],*getdbm(b,matpos(uj,ui^1)));
      bound_badd(pr->tmp[1],*getdbm(b,matpos(uj^1,ui)));
      if (bound_sgn(pr->tmp[0])<=0 &&
				 ...
				 oct_t* a, ap_linexpr0_t* expr)
{
  ...
  if (!a->closed && !a->m) {
    ...
    case BINARY:
      if (u.coef_i==1) ui = 2*u.i; else ui = 2*u.i+1;
      if (u.coef_j==1) uj = 2*u.j; else uj = 2*u.j+1;
      bound_badd(pr->tmp[0],*getdbm(b,matpos2(uj,ui^1)));
      bound_badd(pr->tmp[1],*getdbm(b,matpos2(uj^1,ui)));
      interval_of_bounds(pr,r,pr->tmp[0],pr->tmp[1],false);
      /* exact on Q if closed and no conversion error */
      if (num_incomplete || a->intdim) flag_incomplete;
      else if (!a->closed) flag_algo;
	...
	bounds_mul(pr->tmp[2*i+2],pr->tmp[2*i+3],
		   *getdbm(b,matpos(2*i,2*i+1)),*getdbm(b,matpos(2*i+1,2*i)),
				   ...
				   oct_t* a, ap_dim_t dim)
{
  ...
  if (!a->closed && !a->m) {
    ...
    interval_of_bounds(pr,r,
		       *getdbm(a->closed,matpos(2*dim,2*dim+1)),
		       *getdbm(a->closed,matpos(2*dim+1,2*dim)),true);
    if (num_incomplete || a->intdim) flag_incomplete;
    else if (pr->conv) flag_conv;
  }
  else {
    /* not optimal */
    interval_of_bounds(pr,r,
		       *getdbm(a->m,matpos(2*dim,2*dim+1)),*getdbm(a->m,matpos(2*dim+1,2*dim)),
		       true);
    flag_algo;
  }
  return r;
...
ap_lincons0_array_t oct_to_lincons_array(ap_manager_t* man, oct_t* a)
{
  ...
  if (!a->closed && !a->m) {
    ...
    for (i=0;i<2*a->dim;i++)
      for (j=0;j<=(i|1);j++) {
	if (i==j || bound_infty(*getdbm(m,matpos2(i,j)))) continue;
	ar.p[n] = lincons_of_bound(pr,i,j,*getdbm(m,matpos2(i,j)));
	n++;
      }
    ar.size = n;
  ...
  if (!a->closed && !a->m) {
    ...
    for (i=0;i<a->dim;i++)
      interval_of_bounds(pr,in[i],
			 *getdbm(m,matpos(2*i,2*i+1)),*getdbm(m,matpos(2*i+1,2*i)),true);
