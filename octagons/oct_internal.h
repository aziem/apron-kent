...
#define checked_malloc(ptr,t,nb,action)				\
  ...
  dbm* hmat_alloc       (oct_internal_t* pr, size_t dim);
  void hmat_free        (oct_internal_t* pr, dbm* m, size_t dim);
  dbm* hmat_alloc_zero  (oct_internal_t* pr, size_t dim);
  dbm* hmat_alloc_top   (oct_internal_t* pr, size_t dim);
  dbm* hmat_copy        (oct_internal_t* pr, dbm* m, size_t dim);
  ...
  void myadd				(bound_t res, bound_t b, bound_t temp, dbm* d, size_t index1, size_t index2);
  void myadd2				(bound_t res, bound_t b, short index1);
#endif
  bool is_my_infty			(dbm* m, size_t index);
  short dbm_insert			(bound_t new);
  void setdbm				(dbm* m, size_t k, bound_t new);

  void setdbm_thd			(dbm* m, size_t k, bound_t new);
  void setdbmbmin_thd			(dbm* m, size_t k, bound_t new);

  void setdbmbmin			(dbm* m, size_t k, bound_t b);
  void setdbmmin			(dbm* m, size_t k, bound_t b1, bound_t b2);
  void setdbmbmax			(dbm* m, size_t k, bound_t b);
  void setdbmmax			(dbm* m, size_t k, bound_t b1, bound_t b2);
  void setdbmbadd			(dbm* m, size_t k, bound_t b); 
  void setdbmadd			(dbm* m, size_t k, bound_t b1, bound_t b2);
  void setdbmsub			(dbm* m, size_t k, bound_t b1, bound_t b2);
  void setdbm_mul_2			(dbm* m, size_t k, bound_t b);
  void setdbm_div_2			(dbm* m, size_t k, bound_t b);
  void setdbminfty			(dbm* d, size_t k);
  void setdbmzero			(dbm* d, size_t k);
  bound_t* getdbm			(dbm* d, size_t k);
  bound_t* getvalue			(size_t k);
  void dbm_set_array			(dbm* dst, dbm* src, size_t size);
  void dbm_set_array_from_point		(dbm* dst, dbm* src, size_t point, size_t point2, size_t size);
  void dbm_bound_set_array		(bound_t* dst, dbm* src, size_t size);
  void dbm_bound_set_array2		(dbm* dst, bound_t* src, size_t size);
  size_t dbm_serialized_size_array	(dbm* src, size_t size);
  size_t dbm_serialize_array		(void* dst, dbm* src, size_t size);
  size_t dbm_deserialize_array		(dbm* dst, const void *src, size_t size);
  ...
  /* position of (i,j) element, assuming j/2 <= i/2 */
  static inline size_t matpos_memo(size_t i, size_t j, size_t* memo)
  {
    return j + memo[i];
  }

  /* position of (i,j) element, no assumption */
  static inline size_t matpos2_memo(size_t i, size_t j, size_t* memo)
  {
    if (j>i) return matpos_memo(j^1,i^1, memo);
    else return matpos_memo(i,j, memo);
  }
  ...
  /* bool hmat_close_unary_incremental_nonequality(dbm* m, size_t dim, size_t a, bound_t d); */
  /* bool hmat_close_unary_incremental_equality(dbm* m, size_t dim, size_t a, bound_t d, bound_t dprime); */
...
#error "No numerical type defined"
  ...
  */
  ...
  struct _oct_t {
    ...
    dbm* m;      /* contraint half-matrix (or NULL) */
    /* bound_t* closed; /\* closed version of m (or NULL for not available) *\/ */
    dbm* closed; /* closed version of m (or NULL for not available) */
  ...
  */
  ...
  oct_t* oct_set_mat        (oct_internal_t* pr, oct_t* a, dbm* m,
			     dbm* closed, bool destructive);
