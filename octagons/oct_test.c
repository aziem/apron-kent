...
char check(oct_t* o)
{
  ...
  if (o->closed) {
    ...
    hmat_close(pr, cl,o->dim);
    for (i=0;i<matsize(o->dim);i++)
      if (bound_cmp(*getdbm(cl,i),*getdbm(o->closed,i))) {
...
oct_t* random_oct(int dim,float frac)
{
  ...
  for (i=0;i<2*dim;i++)
    for (j=0;j<=(i|1);j++,b++) {
      ...
      x = lrand48()%20-2;      
      num_set_int2(n,x,y);
      bound_set_num(*getdbm(b,matpos2(i,j)),n);
    }
  num_clear(n);
  return o;
}
...
void test_incremental_closure(void)
{
  ...
  LOOP {
    ...
    if (o->closed) {
      for (i=0;2*i<dim;i++) {
	if (lrand48()%10>8) random_bound(*getdbm(o->closed,matpos2(i,2*v)));
	if (lrand48()%10>8) random_bound(*getdbm(o->closed,matpos2(i,2*v+1)));
      }
