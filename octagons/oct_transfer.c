 ...
 */
bool hmat_add_lincons(oct_internal_t* pr, dbm* b, size_t intdim, size_t dim,
  ...
  for (i=0;i<ar->size;i++) {
   ap_constyp_t c = ar->p[i].constyp;
    ...
    case UNARY:
      ...
      if (c==AP_CONS_EQ) {
      	if (*respect_closure) {
      	  if (hmat_close_binary_incremental_equality(pr, b, dim, ui, ui^1, pr->tmp[1], pr->tmp[0])) return true;
      	} else {
      	  setdbmbmin(b,matpos(ui,ui^1),pr->tmp[1]);
      	  setdbmbmin(b,matpos(ui^1,ui),pr->tmp[0]);
      	}
      } else {
      	if (*respect_closure) {
      	  if (hmat_close_binary_incremental_inequality(pr, b,dim, ui, ui^1, pr->tmp[1])) return true;
      	} else {
      	  setdbmbmin(b,matpos(ui,ui^1),pr->tmp[1]);
      	}
      }
    ...
    case BINARY:
      ...
      if (c==AP_CONS_EQ) {
      	if (*respect_closure) {
      	  if (hmat_close_binary_incremental_equality(pr, b, dim, uj, ui^1, pr->tmp[1], pr->tmp[0])) return true;
      	} else {
      	  setdbmbmin(b,matpos2(uj,ui^1),pr->tmp[1]);
      	  setdbmbmin(b,matpos2(uj^1,ui),pr->tmp[0]);
      	}
      } else {
      	if (*respect_closure) {
      	  if (hmat_close_binary_incremental_inequality(pr, b, dim, uj, ui^1, pr->tmp[1])) return true;
      	} else {
      	  setdbmbmin(b,matpos2(uj,ui^1),pr->tmp[1]);
      	}
      }
	...
	*respect_closure = false; /* do not respect closure */
	...
	for (j=0;j<dim;j++) {
	  bounds_mul(tmpa,tmpb, *getdbm(b,matpos(2*j,2*j+1)),*getdbm(b,matpos(2*j+1,2*j)),
	...
	if (bound_infty(Cb)) ;
	  ...
	  for (j=0;j<dim;j++) {
		...
		!bound_infty(*getdbm(b,matpos(2*j+1,2*j)))) {
	      /* -x <= expr-x <= max(expr) - max x */
	      bound_sub(tmpa,Cb,*getdbm(b,matpos(2*j+1,2*j)));
	      uj = 2*j+1;
	    }
	    else if (bound_cmp_int(pr->tmp[2*j+3],-1)<=0 &&
		     !bound_infty(*getdbm(b,matpos(2*j,2*j+1)))) {
	      /* x <= expr+x <= max(expr) - max(-x) */
	      bound_sub(tmpa,Cb,*getdbm(b,matpos(2*j,2*j+1)));
	      uj = 2*j;
	    }
	    else continue;
	    for (k=j+1;k<dim;k++) {
		  ...
		  !bound_infty(*getdbm(b,matpos(2*k+1,2*k)))) {
		/* (+/-)x -y <= max(expr) - max((+/-)x) - max y */
		bound_sub(tmpb,tmpa,*getdbm(b,matpos(2*k+1,2*k)));
		bound_div_2(tmpb,tmpb);
		setdbmbmin(b,matpos(2*k,uj),tmpb);
	      }
	      else if (bound_cmp_int(pr->tmp[2*k+3],-1)<=0 &&
		       !bound_infty(*getdbm(b,matpos(2*k,2*k+1)))) {
		/* (+/-)x +y <= max(expr) - max((+/-)x) - max (-y) */
		bound_sub(tmpb,tmpa,*getdbm(b,matpos(2*k,2*k+1)));
		bound_div_2(tmpb,tmpb);
		setdbmbmin(b,matpos(2*k+1,uj),tmpb);
	      }
	    }
	  }
	}
	  ...
	  for (k=0;k<dim;k++) {
		...
		!bound_infty(*getdbm(b,matpos(2*k+1,2*k)))) {
	      /* (+/-)x -y <= max(expr) - max((+/-)x) - max y */
	      bound_sub(tmpb,Cb,*getdbm(b,matpos(2*k+1,2*k)));
	      bound_div_2(tmpb,tmpb);
	      setdbmbmin(b,matpos2(2*k,uj),tmpb);
	    }
	    else if (bound_cmp_int(pr->tmp[2*k+3],-1)<=0 &&
		     !bound_infty(*getdbm(b,matpos(2*k,2*k+1)))) {
	      /* (+/-)x +y <= max(expr) - max((+/-)x) - max (-y) */
	      bound_sub(tmpb,Cb,*getdbm(b,matpos(2*k,2*k+1)));
	      bound_div_2(tmpb,tmpb);
	      setdbmbmin(b, matpos2(2*k+1,uj),tmpb);
	    }
	  }
	}
	  ...
	  bound_div_2(tmpa,Cb);
	  setdbmbmin(b,matpos2(uj^1,ui),tmpa);
	}
...
void hmat_add_generators(oct_internal_t* pr, dbm* b, size_t dim,
			 ap_generator0_array_t* ar)
{
  ...
  for (i=0;i<ar->size;i++) {
	...
	setdbmbmax(bb, matpos2(j,k),pr->tmp[2*dim]);
      }
  }
...
static inline void hmat_forget_var(dbm* m, size_t dim, size_t d)
{
  size_t k;
  for (k=0;k<2*d;k++) {
    setdbminfty(m,matpos(2*d,k));
    setdbminfty(m,matpos(2*d+1,k));
  }
  for (k=2*d+2;k<2*dim;k++) {
    setdbminfty(m,matpos(k,2*d));
    setdbminfty(m,matpos(k,2*d+1));
  }
  setdbminfty(m, matpos(2*d,2*d+1));
  setdbminfty(m, matpos(2*d+1,2*d));
}
 ...
 */
static void hmat_assign(oct_internal_t* pr, uexpr u, dbm* m, size_t dim,
			size_t d, bool* respect_closure)
{
  ...
  if (u.type==ZERO ) {
    ...
    if (*respect_closure) {
      ...
      for (k=0;k<2*d;k++) {
	bound_div_2(pr->tmp[2],*getdbm(m,matpos(k^1,k)));
	setdbmadd(m,matpos(2*d,k),pr->tmp[2],pr->tmp[0]);
	setdbmadd(m,matpos(2*d+1,k),pr->tmp[2],pr->tmp[1]);
      }
      for (k=2*d+2;k<2*dim;k++) {
	bound_div_2(pr->tmp[2],*getdbm(m,matpos(k,k^1)));
	setdbmadd(m,matpos(k,2*d),pr->tmp[2],pr->tmp[1]);
	setdbmadd(m,matpos(k,2*d+1),pr->tmp[2],pr->tmp[0]);
      }
    }
    else {
      /* plain version */
    ...
    setdbm_mul_2(m,matpos(2*d,2*d+1),pr->tmp[0]);
    setdbm_mul_2(m,matpos(2*d+1,2*d),pr->tmp[1]);
  }
    ...
    if (*respect_closure) {
      hmat_close_binary_incremental_inequality(pr, m,dim,2*d,i, pr->tmp[0]);
      hmat_close_binary_incremental_inequality(pr, m,dim,i,2*d, pr->tmp[1]);
    } else {
      setdbm(m, matpos2(2*d,i), pr->tmp[0]);
      setdbm(m, matpos2(i,2*d), pr->tmp[1]);
    }
  }
    ...
    for (k=0;k<2*d;k++) {
      bound_set(pr->tmp[2],*getdbm(m,matpos(2*d,k)));
      setdbmadd(m,matpos(2*d,k),*getdbm(m,matpos(2*d+1,k)),pr->tmp[0]);
      setdbmadd(m,matpos(2*d+1,k),pr->tmp[2],pr->tmp[1]);
    }
    for (k=2*d+2;k<2*dim;k++) {
      bound_set(pr->tmp[2],*getdbm(m,matpos(k,2*d)));
      setdbmadd(m,matpos(k,2*d),*getdbm(m,matpos(k,2*d+1)),pr->tmp[1]);
      setdbmadd(m,matpos(k,2*d+1),pr->tmp[2],pr->tmp[0]);
    }
    bound_mul_2(pr->tmp[0],pr->tmp[0]);
    bound_mul_2(pr->tmp[1],pr->tmp[1]);
    bound_set(pr->tmp[2],*getdbm(m,matpos(2*d+1,2*d)));
    setdbmadd(m,matpos(2*d+1,2*d),*getdbm(m,matpos(2*d,2*d+1)),pr->tmp[1]);
    setdbmadd(m,matpos(2*d,2*d+1),pr->tmp[2],pr->tmp[0]);
  }
    ...
    /* always respect closure */
    for (k=0;k<2*d;k++) {
      setdbmbadd(m,matpos(2*d,k),pr->tmp[0]);
      setdbmbadd(m,matpos(2*d+1,k),pr->tmp[1]);
    }
    for (k=2*d+2;k<2*dim;k++) {
      setdbmbadd(m,matpos(k,2*d),pr->tmp[1]);
      setdbmbadd(m,matpos(k,2*d+1),pr->tmp[0]);
    }
    bound_mul_2(pr->tmp[0],pr->tmp[0]);
    bound_mul_2(pr->tmp[1],pr->tmp[1]);
    setdbmbadd(m,matpos(2*d+1,2*d),pr->tmp[1]);
    setdbmbadd(m,matpos(2*d,2*d+1),pr->tmp[0]);
  }
    ...
    *respect_closure = false; /* does not respect closure */
    ...
    bound_mul_2(Cb,pr->tmp[1]);
    for (i=0;i<dim;i++) {
      bounds_mul(tmpa,tmpb, *getdbm(m,matpos(2*i,2*i+1)),*getdbm(m,matpos(2*i+1,2*i)),
    ...
    if (!Cinf) {
      ...
      setdbm(m, matpos(2*d+1, 2*d), Cb);
      for (i=0;i<dim;i++)  {
	if (i==d) continue;
	if (bound_cmp_int(pr->tmp[2*i+2],-1)<=0 &&
	    !bound_infty(*getdbm(m,matpos(2*i+1,2*i)))) {
	  /* bound for x-y */
	  bound_sub(tmpa,Cb,*getdbm(m,matpos(2*i+1,2*i)));
	  setdbm_div_2(m,matpos2(2*i,2*d),tmpa);
	}
	else if (bound_cmp_int(pr->tmp[2*i+3],-1)<=0 &&
		 !bound_infty(*getdbm(m,matpos(2*i,2*i+1)))) {
	  /* bound for x+y */
	  bound_sub(tmpa,Cb,*getdbm(m,matpos(2*i,2*i+1)));
	  setdbm_div_2(m,matpos2(2*i+1,2*d),tmpa);
	}
      }
    }
    else if (Cinf==1) {
	...
	if (!bound_cmp_int(pr->tmp[2*i+3],1) &&
	  ...
	  setdbm_div_2(m,matpos2(2*Ci,2*d),Cb);
	else if (!bound_cmp_int(pr->tmp[2*i+3],-1) &&
		 !bound_cmp_int(pr->tmp[2*i+2],1))
	  setdbm_div_2(m,matpos2(2*Ci+1,2*d),Cb);
      }
    }
	...
	if (bound_cmp_int(pr->tmp[2*i+2],-1)<=0 &&
	  ...
	  /* bound for -x+y */
	  bound_sub(tmpa,cb,*getdbm(m,matpos(2*i,2*i+1)));
	  setdbm_div_2(m,matpos2(2*i+1,2*d+1),tmpa);
	}
	else if (bound_cmp_int(pr->tmp[2*i+3],-1)<=0 &&
		 !bound_infty(*getdbm(m,matpos(2*i+1,2*i)))) {
	  /* bound for -x-y */
	  bound_sub(tmpa,cb,*getdbm(m,matpos(2*i+1,2*i)));
	  setdbm_div_2(m,matpos2(2*i,2*d+1),tmpa);
	}
      }
    }
    else if (cinf==1) {
	...
	if (!bound_cmp_int(pr->tmp[2*i+3],1) &&
	    !bound_cmp_int(pr->tmp[2*i+2],-1))
	  setdbm_div_2(m,matpos2(2*d,2*ci),cb);
	else if (!bound_cmp_int(pr->tmp[2*i+3],-1) &&
		 !bound_cmp_int(pr->tmp[2*i+2],1))
	  setdbm_div_2(m,matpos2(2*d,2*ci+1),cb);
      }
    }
...
static bool hmat_subst(oct_internal_t* pr, uexpr u, dbm* m, size_t dim,
		       size_t d, dbm* dst, bool* respect_closure)
{
#if defined(OCTINCRDEBUG)
  startfuncdebug(__func__);
#endif
  ...
  if (u.type==ZERO ) {
    ...
    *respect_closure = false; /* TODO: does it respect closure? */
    ...
    bound_add(pr->tmp[2],pr->tmp[2],*getdbm(m,matpos(2*d+1,2*d)));
    bound_add(pr->tmp[3],pr->tmp[3],*getdbm(m,matpos(2*d,2*d+1)));
    if (bound_sgn(pr->tmp[2])<0 || bound_sgn(pr->tmp[3])<0) return true;

    /* infer unary contraints cX from binary constraints on cX + c'Xd */
    for (i=0;i<2*d;i++) {
      bound_add(pr->tmp[2],pr->tmp[0],*getdbm(m,matpos(2*d+1,i^1)));
      bound_add(pr->tmp[3],pr->tmp[1],*getdbm(m,matpos(2*d,i^1)));
      bound_mul_2(pr->tmp[2],pr->tmp[2]);
      bound_mul_2(pr->tmp[3],pr->tmp[3]);
      setdbmbmin(m,matpos(i,i^1),pr->tmp[2]);
      setdbmbmin(m,matpos(i,i^1),pr->tmp[3]);
    }
    for (i=2*d+2;i<2*dim;i++) {
      bound_add(pr->tmp[2],pr->tmp[0],*getdbm(m,matpos(i,2*d)));
      bound_add(pr->tmp[3],pr->tmp[1],*getdbm(m,matpos(i,2*d+1)));
      bound_mul_2(pr->tmp[2],pr->tmp[2]);
      bound_mul_2(pr->tmp[3],pr->tmp[3]);
      setdbmbmin(m,matpos(i,i^1),pr->tmp[2]);
      setdbmbmin(m,matpos(i,i^1),pr->tmp[3]);
    ...
    *respect_closure = false; /* TODO: does it respect closure? */
    ...
    /* test satisfiability */
    bound_add(pr->tmp[2],pr->tmp[0],*getdbm(m,matpos2(k,2*d)));
    bound_add(pr->tmp[3],pr->tmp[1],*getdbm(m,matpos2(2*d,k)));
    if (bound_sgn(pr->tmp[2])<0 || bound_sgn(pr->tmp[3])<0) return true;

    /* infer binary constraints by substitution */
    for (i=0;i<2*d;i++) {
      bound_add(pr->tmp[2],pr->tmp[0],*getdbm(m,matpos(2*d+1,i)));
      bound_add(pr->tmp[3],pr->tmp[1],*getdbm(m,matpos(2*d,i)));
      setdbmbmin(m,matpos2(k^1,i),pr->tmp[2]);
      setdbmbmin(m,matpos2(k,i),pr->tmp[3]);
    }
    for (i=2*d+2;i<2*dim;i++) {
      bound_add(pr->tmp[2],pr->tmp[0],*getdbm(m,matpos(i^1,2*d)));
      bound_add(pr->tmp[3],pr->tmp[1],*getdbm(m,matpos(i^1,2*d+1)));
      setdbmbmin(m,matpos2(k^1,i),pr->tmp[2]);
      setdbmbmin(m,matpos2(k,i),pr->tmp[3]);
    }
    ...
    bound_mul_2(pr->tmp[2],pr->tmp[0]);
    bound_mul_2(pr->tmp[3],pr->tmp[1]);
    bound_badd(pr->tmp[2],*getdbm(m,matpos(2*d+1,d*2)));
    bound_badd(pr->tmp[3],*getdbm(m,matpos(2*d,d*2+1)));
    setdbmbmin(m,matpos(k^1,k),pr->tmp[2]);
    setdbmbmin(m,matpos(k,k^1),pr->tmp[3]);
...
static void hmat_subst_var(dbm* m, size_t dim, size_t d, size_t k)
{
#if defined(OCTINCRDEBUG)
  startfuncdebug(__func__);
#endif
  ...
  /* infer binary constraints by substitution */
  for (i=0;i<2*d;i++) {
    setdbmbmin(m,matpos2(2*k+1,i),*getdbm(m,matpos(2*d+1,i)));
    setdbmbmin(m,matpos2(2*k,i),*getdbm(m,matpos(2*d,i)));
  }
  for (i=2*d+2;i<2*dim;i++) {
    setdbmbmin(m,matpos2(2*k+1,i),*getdbm(m,matpos(i^1,2*d)));
    setdbmbmin(m,matpos2(2*k,i),*getdbm(m,matpos(i^1,2*d+1)));
  }

  /* infer unary constraints by substitution */
  setdbmbmin(m,matpos(2*k+1,2*k),*getdbm(m,matpos(2*d+1,d*2)));
  setdbmbmin(m,matpos(2*k,2*k+1),*getdbm(m,matpos(2*d,d*2+1)));
  hmat_forget_var(m,dim,d);
}
			  ...
			  oct_t* dest)
{
  ...
  /* intersect with dest */
  if (dest) {
    dbm* m2 = dest->closed ? dest->closed : dest->m;
    size_t i;
    for (i=0;i<matsize(a->dim);i++)
      setdbmbmin(m,i,*getdbm(m2,i));
  }
			      ...
			      oct_t* dest)
{
  ...
  /* go */
  if (hmat_subst(pr,u,m,a->dim,d,m2,&respect_closure)) {
    /* empty */
    if (!destructive) hmat_free(pr,m,a->dim);
    return oct_set_mat(pr,a,NULL,NULL,destructive);
  }
				...
				oct_t* dest)
{
  ...
  dbm_set_array(mm,m,matsize(a->dim));
  ...
  if (pr->funopt->algorithm>=0) {
    if (hmat_close(pr, mm,a->dim+size)) {
      /* empty */
      hmat_free(pr,mm,a->dim+size);
      return oct_set_mat(pr,a,NULL,NULL,destructive);
    }
				    ...
				    oct_t* dest)
{
  ...
  for (i=0;i<size;i++) {
    ...
    for (j=0;j<src;j++) {
      setdbmbmin(mm,matpos(dst+1,j),*getdbm(mm,matpos(src+1,j)));
      setdbmbmin(mm,matpos(dst,j),*getdbm(mm,matpos(src,j)));
    }
    for (j=src+2;j<2*a->dim+2*size;j++) {
      setdbmbmin(mm,matpos2(dst+1,j),*getdbm(mm,matpos(j^1,src)));
      setdbmbmin(mm,matpos2(dst,j),*getdbm(mm,matpos(j^1,src+1)));
    }
    setdbmbmin(mm,matpos(dst+1,dst),*getdbm(mm,matpos(src+1,src)));
    setdbmbmin(mm,matpos(dst,dst+1),*getdbm(mm,matpos(src,src+1)));
  ...
  else dbm_set_array(m,mm,matsize(a->dim));
  hmat_free(pr,mm,a->dim+size);
