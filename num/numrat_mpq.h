...
{
  /* if(mpz_cmp(mpq_denref(a), mpq_denref(b)) == 0) { */
  /*   return mpz_cmp(mpq_numref(a), mpq_numref(b)); */
  /* } */
  if(numint_equal(numrat_denref(a), numrat_denref(b))) {
      return numint_cmp(numrat_numref(a), numrat_numref(b));
  }
  return mpq_cmp(a,b);
